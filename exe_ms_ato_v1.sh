...
if [ $# -eq 0 ]; then
   echo "Usage: $0 <number of stack> <optional: number of checkpoint>"
   exit 1
fi
# if [ -z "$1" ]; then
...
NVM_START=0x440000000 # 17G(0x440000000) # 17G+256MB(0x450000000)
NVM_SIZE=16GB
...
PIM_SPM_START=0x850000000 # Must be after the SE memory range
PIM_SPM_SIZE=4kB # The minimum memory size is page size, but it won't actually be used so much
PIM_SPM_REG_FLUSH_ADDR=0x850000000
PIM_SPM_REG_FLUSH_SIZE=0x850000008
PIM_SE_MEM_START=0x840000000 # Must be after the host physical memory range
...
# CMDLINE="earlyprintk=ttyS0 console=ttyS0 lpj=7999923 root=/dev/hda1 nokaslr norandmaps memmap=16G!17G+256M"
CMDLINE="earlyprintk=ttyS0 console=ttyS0 lpj=7999923 root=/dev/hda1 nokaslr norandmaps memmap=16G!17G"

# DISK_IMAGE=x86-example.img
#DISK_IMAGE=/home/johnnylab/work/gem5_images/x86-filebench_syscall.img
# DISK_IMAGE=/home/oslab-gem5-1/chung_workspace/sim_gem5/master_work/gem5_images/x86-system/disks/x86-example.img
DISK_IMAGE=./master_work/gem5_images/x86-system/disks/x86-ubuntu-14.04.6_v2.img
...
if [ "$#" -gt 1 ]; then
 "$GEM5_TARGET" \
    ...
    --checkpoint-restore="$CHECKPOINT_NUM" \
